...
- Override stylesheet order is guaranteed: Bootswatch base -> shared `light.css|dark.css` -> per-theme `<theme>.css` -> shared/app CSS. Mode-only switches do not reload the base theme or call `renderMain()`/network refresh; they update attributes, override links, contrast variables, and images in place.
...
- Screenshot generation was not run after adopting the matrix because it is intentionally expensive; syntax and helper tests were run. The user will run the screenshot suites manually. ALWAYS generate screenshots with `--workers 16`; this is the documented standard for every `tests/*-screenshots.spec.js` command.
...
- PlanMyDay targeted batches passed: Streams 27, Search 15, Today/font/geometry 12, task/schedule 4, touch 5, and theme/header contrast checks. The final full PlanMyDay regression and the new screenshot generation matrix are intentionally left for the user's manual run.
