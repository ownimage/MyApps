...
13: INNERHTML-PARSE CONNECT RACE (2026-09-19): on Chromium, `host.innerHTML = '<my-el …>'` where `host` is ALREADY connected connects the custom element DURING the parse — each `attributeChangedCallback` fires with `isConnected === true` BEFORE `connectedCallback` runs, so a template stamped only in `connectedCallback` is not present yet and `this.querySelector('.x').textContent = …` in `_render()` throws "Cannot set properties of null (setting 'textContent')" (41 stack-less pageerrors in the storybook probe, from the pmd cards + smd-image-picker during `renderAll`). Fix: guard every `attributeChangedCallback` re-render with the component's bound flag — `if (this.isConnected && this._bound) this._render();` (the end of `connectedCallback` still does the initial render). Applied to all light-DOM components sharing the pattern: `pmd-job-today-card`, `pmd-stream-header`, `pmd-stream-job-card`, `pmd-job-search-card`, `smd-image-picker`, `smd-draghandle`, `smd-theme`, `cmd-date-card`, `cmd-countdown-card`, `cmd-category-card`, `qrlink-card`, `solar-top-tiles` (this last one gained a `_bound` flag). Components already safe: `_built`-guarded (`smd-image-card`, `smd-image-select`, `smd-image-dropdown`, `smd-date-picker`); null-guarded render queries (`smd-image`, `smd-button`, `smd-checkbox` via `_input`, `smd-badge`, `smd-buymeacoffee`, `smd-qr-export`, `smd-qrcode`, `smd-page`, `smd-tabs`, `smd-qr-import`).
...
### 2026-09-19 — `pmd-job-search-card` always uses the shared image prefix
- Removed the `key-prefix` attribute from `pmd-job-search-card` (and the
  `card.setAttribute("key-prefix", …)` in `PlanMyDay/js/job-search.js`
  `buildJobSearchCard`): the card always reads from the shared library
  (`smdImagePrefix()` → `"shared-"`), so the attribute was redundant with the
  template's hardcoded `key-prefix="shared-"`. Dropped `key-prefix` from
  `observedAttributes` and the `sImg.setAttribute('key-prefix', …)` in
  `_render()` `setThumb`.
- Note: the sibling `pmd-stream-header` / `pmd-stream-job-card` /
  `pmd-job-today-card` still keep their `key-prefix` attribute + fallback —
  only this card was asked to drop it.
- Verified: `node --check` both files; `--grep "search"` batch 24/24 green
  (Search Jobs, Images Editor, Image Picker). `BUILD_NUMBER` unchanged (user
  ships).

### 2026-09-19 — pmd-job-today-card cosmetics + cross-app CSS consolidation
- `pmd-job-today-card`: title `<h4 class="title">` → `<h2 class="title">` (h2
  token instead of h1; pixel literals in `pmd-touch.spec.js` updated 41.6→26 /
  51.2→32, compact 25.6 unchanged); `.stream-title` now uses the p token
  (`--smd-type-p`) instead of h2; `.job-view-btn` no longer bold; the tab badge
  now renders BEFORE the View button in `.meta-row`. h2-related regression
  assertions updated (`h4`→`h2` locators, badge-before-View x-order swap).
  Cosmetic — user deferred the full regression run.
- CSS consolidation (user rule: rules whose selectors mention no app-specific
  component move to `shared/css/styles.css`; PlanMyDay's values win clashes):
  moved `#countdownContainer` flex layout + `> .flex-shrink-0`, `#imagesEditor`,
  `body.compact .card/.card.p-3/#countdownContainer .mb-3/.editor-btn/.btn-wide`
  (density), and SolarControlar's `.flash*` into shared. Deleted dead PMD rules:
  `.countdown-card` blocks (today/CMD cards are `pmd-*`/`cmd-*` components now —
  zero markup uses the class), `.type-select`/`.date-day-select`/
  `.date-month-select`/`.flatpickr-date` (date selects don't use them). Removed
  FreeFormOX duplicates already identical in shared (`#mainNav` safe-area,
  `.build-number`, `#settingsPage` padding, `.editor-btn`, `.btn-wide`).
- BEHAVIOUR NOTE: moving `#countdownContainer`/compact-density to shared means
  CountMyDays/QRLinks now adopt PlanMyDay's flex-column full-height container
  and PMD's compact `editor-btn`/`btn-wide` widths — a visual change for those
  apps. Targeted smoke (CMD boots/renders, QR empty state, touch title font
  sizes, today-card layout, adhoc/hidden/sort tests) all green; full suites
  still to be run. `BUILD_NUMBER` unchanged (user ships).
