...
- `shared/css/styles.css` is now a mechanics-only stylesheet. It retains state selectors, touch/scroll behavior, smd-page slide transforms/transitions, modal open/close positioning, tab panel visibility, dropdown positioning, and minimal layout needed for controls; typography, color, background, border, shadow, and decorative visual overrides were removed. Bootstrap/Bootswatch and app-specific CSS own appearance.
- Do not reintroduce shared font-size ramps, contrast/palette rules, or decorative component skins. App-specific component CSS and theme stylesheets are the correct locations for intentional visual exceptions.
