...
function renderMain() {
  ...
  allJobs.forEach(({ job, streamIdx, jobIdx }) => {
    ...
    // The card takes the whole job + stream objects and derives its own display
    // fields (title, suffix, daily icon, images, tab, description) from them.
    card.job = job;
    card.stream = stream;
