...
// are stable so tests can target them with plain selectors. The flatpickr
// calendar is appended to document.body so its stylesheet (loaded in the page)
// still applies.
(function (global) {
  "use strict";
...
`;
  ...
  class SmdDatePicker extends HTMLElement {
    ...
    _bind() {
      ...
      this._bound = true;
      const btn = this.querySelector('#smdDatePickerClearBtn');
      if (!btn) return;
      this._onClearClick = () => {
        this._value = "";
        this._sync();
    ...
    _unbind() {
      ...
      this._bound = false;
      const btn = this.querySelector('#smdDatePickerClearBtn');
      if (btn) btn.removeEventListener('click', this._onClearClick);
    ...
    _applyAttrs() {
      ...
      const raw = this.querySelector('#smdDatePickerInput');
      if (!raw) return;
      raw.placeholder = placeholder;
      raw.readOnly = true;
      raw.disabled = disabled;
      const alt = this.querySelector('#smdDatePickerAlt');
      if (alt) {
        alt.placeholder = placeholder;
        alt.disabled = disabled;
      }
      const clearBtn = this.querySelector('#smdDatePickerClearBtn');
      if (clearBtn) {
  ...
  if (!global.customElements.get('smd-date-picker')) {
    global.customElements.define('smd-date-picker', SmdDatePicker);
  }
  global.SmdDatePicker = SmdDatePicker;
})(window);